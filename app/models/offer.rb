class Offer < ApplicationRecord
  ...
  class << self
    def perfect_match_offers(user_departments, offers, departments, query)
      scope = Offer
      scope = scope.joins(offer_departments: :department).where('offer_departments.offer_id IN (?)', offers.pluck(:id))
      scope = scope.where("departments.id": departments) if departments.present?
    ...
    def good_match_offers(offers, departments, query)
      scope = offers.joins(:departments)
      scope = scope.where("departments.id": departments) if departments.present?
      scope = scope.where("offers.company like ?", "%#{query}%") if query.present?
